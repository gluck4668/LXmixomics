...
LXmixomics_gene_meta <- function(gene_data,meta_data){

  # 建立文件夹
  dir_name <- "analysis results_gene_meta"
  ...
  meta_df <- limma::avereps (meta_df[,-1],ID = meta_df[,1])%>% data.frame()
  meta_df <- dplyr::filter_all(meta_df,any_vars(.!=0)) %>% t() %>% data.frame() #去掉全部数据为0的行

  # 判断组名是否相同
  group_names <- rownames(gene_df)==rownames(meta_df)
  ...
  list.keepX = list(genes = rep(8,ng), metabolites = rep(8,ng))
